...
class MyClient(discord.Client):
    ...
    async def on_message(self, message):    
        ...
        if message.author == client.user:
            return
        if message.channel.id == 840490174684069938:

            new_user_input_ids = tokenizer.encode(message.content + tokenizer.eos_token, return_tensors='pt').to("cuda")
            print(step)
            print(chat_ids)
            ...
            chat_ids = model.generate(bot_input_ids, max_length=1000,num_beam_groups=4, num_beams=4, diversity_penalty=1.0, pad_token_id=tokenizer.eos_token_id).to("cuda")
